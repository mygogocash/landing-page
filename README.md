...
```bash
cd functions && npm ci && cd ..   # required whenever functions/deps change
...
| Firebase deploy: “Couldn't find `firebase-functions`” / unexpected error when deploying functions | Run `cd functions && npm ci` (or `npm install`) so `functions/node_modules` exists before `npm run deploy:firebase:full`. |
